...
- 3 of 7 are curated into machine-readable evidence (`knowledge/curated/`), each carrying
  ...
  including the honestly-disclosed 2-source curation gap.
...
Current state (branch `feat/d5-d7-service-resolution-readiness`, base `main` @ `3c7f3e5`):

| Metric | Result | Source |
|---|---|---|
| Retrieval top-1 accuracy | 10/10 | `evals/golden_questions_results.json` |
| Retrieval top-3 accuracy | 10/10 | `evals/golden_questions_results.json` |
| Journey classification accuracy | 10/10 | `evals/routing_escalation_results.json` |
| Escalation-trigger accuracy | 10/10 | `evals/routing_escalation_results.json` |
| Escalation-target accuracy | 1/1 (only 1 of 10 golden questions currently escalates) | `evals/routing_escalation_results.json` |
...
corpus-scaling limitation of the Day 4 exclusive-term weighting fix; GQ-02's retrieval
fragility; the disclosed 2-of-7-source curation gap affecting `accessibility_inclusion`
and `financial_support`; and, most fundamentally, no real-data or live-workflow
validation of any kind to date.
