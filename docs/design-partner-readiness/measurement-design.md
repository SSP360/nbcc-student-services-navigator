...
| **Content-gap frequency** | How often does a query find no good source? (Directly relevant given the `accessibility_inclusion`/`financial_support` gap in `service-resolution-traces.md`.) | Aggregate count of `unmatched_query` or low-confidence-score events, with query *topic* categorized, not query *text* retained. |
